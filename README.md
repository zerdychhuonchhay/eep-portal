...
To run this application on a new local machine:
...
Build Locally: Write and test new code on your local laptop using VS Code (flask run).

Push Code: Once a feature works perfectly, send it to the GitHub vault:
...
Pull to Production: Open the PythonAnywhere Bash Console and pull the new code down to the live server:

git pull


Reload: Click the green "Reload" button on the PythonAnywhere Web Dashboard to immediately make the updates live for the NGO staff.
